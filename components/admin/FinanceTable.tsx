          ...
          {FILTERS.map(opt => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-1.5 cursor-pointer ${isActive ? ACTIVE_CLASS[opt.value] : "text-gray-500 hover:text-gray-800 hover:bg-gray-200/50"
                  }`}
              >
                {opt.label}
              ...
              <tr key={trx.uniqueKey} className="hover:bg-indigo-50/20 transition-colors group">
                ...
                <td className="px-6 py-5 min-w-[220px]">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {trx.subtype === "return" && <Package className="w-4 h-4 text-gray-400 shrink-0" />}
                ...
                <td className={`px-6 py-5 text-right font-black text-base ${AMOUNT_CLASS[trx.subtype] || "text-gray-600"}`}>
                  {trx.type === "income" ? "+" : trx.type === "return" ? "-" : "-"}฿{trx.amount.toLocaleString("th-TH", { minimumFractionDigits: 2 })}
                </td>
              </tr>
            ))}
          </tbody>
